...
hard_voting <- function(listOfdataframe){
  HVdata <- listOfdataframe[[1]]
  
  for (d in 1:ncol(HVdata)) {

    columnstoHV <- data.frame(matrix(NA, nrow = nrow(HVdata), 
                                     ncol = length(listOfdataframe)))
    picked <- sapply(listOfdataframe, `[`, d)
    ...
    #continuous variable
      ...
      HVdata[, d] <- as.numeric(apply(columnstoHV, 1, 
                                      mean))
    }
  }
  return(HVdata)
